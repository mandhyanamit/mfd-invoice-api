...
def process(sources, out_dir, sig_above="", sig_below="", sig_image="",
    ...
    if write_gstr1:
        ...
        stem = (merged_filename(fmt, lo, hi, pad)[:-4] if fmt
                else f"Invoices_{lo:03d}-{hi:03d}")
        json_path = os.path.join(out_dir, f"GSTR1_{stem}.json")
        write_gstr1_json(all_inv, json_path, period)
        written.append(json_path)
        log(f"  written: {os.path.basename(json_path)}")
